...
Worker 只持有 consumer token,真实的 API 密钥和 GitHub PAT 存储在 Higress 网关。即使 Worker 被攻破,也无法泄露真实凭证。
...
```
hiclaw/
├── manager/              # Manager Agent 容器(一体化: Higress + Tuwunel + MinIO + Element Web)
│   ├── agent/           # Agent 配置和技能
...
5. **higress-gateway-management**: AI 网关管理
6. **task-coordination**: 任务协调
7. **project-management**: 项目管理
8. **model-switch**: 模型切换
9. **worker-model-switch**: Worker 模型切换
...
格式:每个逻辑更改一个要点,带提交哈希:
```markdown
- feat(manager): add new skill for task distribution ([a1b2c3d](https://github.com/higress-group/hiclaw/commit/a1b2c3d))
- fix(worker): correct heartbeat interval calculation ([e4f5g6h](https://github.com/higress-group/hiclaw/commit/e4f5g6h))
```
...
**关键技术细节**:
- Qwen Provider 需要 `rawConfigs` 字段(包含 `qwenEnableCompatible` 等)
- Higress Console 使用 Session Cookie 认证(非 Basic Auth)
- MCP Server 通过 `PUT` 创建(非 `POST`)
- Auth 插件首次配置后需要 ~40s 激活
...
所有基础镜像托管在 Higress 阿里云镜像仓库,并设有区域镜像站:
...
| Higress: `Missing Qwen specific configurations` | `type=qwen` 需要 `rawConfigs` 字段 | 包含 `rawConfigs: {qwenEnableCompatible: true, ...}` |
