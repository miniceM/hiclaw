...
**安全设计**：Worker 通过环境变量注入获得 LLM API Key 和 MCP 凭据，不存储在配置文件或文件系统中。即使 Worker 容器被攻击，攻击者只能访问运行时内存，无法获取持久化的凭证。
...
**一条命令启动**：一个 `curl | bash` 搞定所有组件——Matrix 服务器、文件存储、Web 客户端和 Manager Agent 本身。
...
```bash
bash <(curl -sSL https://raw.githubusercontent.com/miniceM/hiclaw/main/install/hiclaw-install.sh)
```
...
```powershell
Set-ExecutionPolicy Bypass -Scope Process -Force; Invoke-Expression ((New-Object System.Net.WebClient).DownloadString('https://raw.githubusercontent.com/miniceM/hiclaw/main/install/hiclaw-install.ps1'))
```
...
```bash
HICLAW_VERSION=0.2.0 bash <(curl -sSL https://raw.githubusercontent.com/miniceM/hiclaw/main/install/hiclaw-install.sh)
```
...
```
Manager 容器
    (持有 LLM API Key、GitHub Token 等真实凭证)
         ↓
    通过 Docker --env 注入环境变量
         ↓
Worker 容器
    (运行时从环境变量读取凭证)
         ↓
    直接访问 LLM API / GitHub API / MCP Server
```

凭证管理：
- LLM API Key 和 MCP 凭据存储在 Manager 容器的环境变量中
- Worker 创建时通过 `docker run --env` 注入必要的环境变量
- Worker 的 `openclaw.json` 使用 `$HICLAW_LLM_API_KEY` 语法，指示 OpenClaw 在运行时从环境变量读取
...
| 凭证管理 | 每个 Agent 持有真实 Key | Worker 通过环境变量注入，不存储 |
...
目前 Worker 通过本地 mcporter 访问 GitHub MCP Server，GitHub Token 通过环境变量注入，不会暴露给 Worker。这个安全的模式可以扩展到任意 MCP 服务：

- **预置 MCP 连接器**：GitHub、Slack、Notion、Linear 等常用服务
- **自定义 MCP 集成**：接入自己的 MCP 服务，通过环境变量注入凭据
